      ...
      <div v-if="!inChat">
        ...
        <div>
          <div v-for="(chat,index) in chats" :key="index" class="test">
            <q-item class="q-my-sm" clickable v-ripple @click.stop="openChat(chat)" style="position: relative;">
              ...
              <div class="flex flex-center">
                <q-icon name="fiber_manual_record"
                        :color="$lget($lget(chat, '_fastjoin.users', []).filter((chatUser) => chatUser._id !== user._id), '[0].active', false) ? 'positive' : 'negative'"
              ...
              <q-item-section>
                <q-item-label>
                  {{
                    chat._fastjoin.users[0]._id === user._id ? $lget(chat, '_fastjoin.users[1].username', $lget(chat, '_fastjoin.users[1].email', 'User Deleted')) : $lget(chat, '_fastjoin.users[0].username', $lget(chat, '_fastjoin.users[0].email', 'User Deleted'))
                  }}
                </q-item-label>
                ...
                <q-btn icon="close" color="red" round size="xs" style="position: absolute; top: 0; right: 5px;z-index: 5;" @click.stop="removeFromChat(chat)"></q-btn>
            </q-item>
            <q-separator/>
          </div>
        </div>
      ...
      <div v-if="inChat" style="display: flex; flex-direction: column; justify-content: center;">
        ...
        >
          <q-chat-message
            ...
            class="q-ma-sm"
          />
          <div class="text-grey-7 text-subtitle2 text-center" v-if="this.inChat.users.filter((inChatUser) => {return inChatUser.left;}).length">{{$lget(notUser, 'username', $lget(notUser, 'email', 'User Deleted'))}} left the chat</div>
        </div>
        <div>
    ...
    methods: {
      ...
      removeFromChat(chat){
        this.$q.dialog({
          ...
          message: 'You will not be able to see messages from this chat after leaving.',
          cancel: true,
          ok: {
            push: true,
            color: 'secondary',
          ...
          if(this.user.chats.includes(chat._id)){
            let removeChatUser = this.user.clone();
            removeChatUser.chats = removeChatUser.chats.filter((filteredChat) => {
              return filteredChat !== chat._id;
            });
            chat.users.map((userInChat) => {
              if(userInChat.user === this.user._id){
                 userInChat.left = true;
              }
            });
            chat.save().then((res) => {
              console.log('res', res);
              removeChatUser.save().then(() => {
                this.$q.notify({
                  color: 'primary',
                  textColor: 'white',
                  icon: 'check_circle',
              ...
              console.error('error on chat save', err);
            })
          }
        })
      },
